...
class SaleOrder(models.Model):
    ...
    def _compute_calculation_trailer(self):
        for rec in self:
            dry_trailer_count = rec.order_line.filtered(lambda x: 'Virtual Interior Design' in x.product_id.name)
    ...
    def _compute_calculation_delivery(self):
        for rec in self:
            dry_delivery_count = rec.order_line.filtered(lambda x: 'Virtual Interior Design' in x.product_id.name)
    ...
    def _compute_calculation_pickup(self):
        for rec in self:
            dry_pickup_count = rec.order_line.filtered(lambda x: 'Virtual Home Staging' in x.product_id.name)
            # rec.dry_delivery = sum(dry_delivery_count.mapped('product_uom_qty'))
